# Forked from https://github.com/tensorflow/models/blob/v2.15.0/official/vision/ops/augment.py#L2351-L2498
#
...
def _normalize_tuple(value, n, name):
    ...
    if isinstance(value, int):
        ...
        try:
            ...
            raise TypeError(
                f"The {name} argument must be a tuple of {n} integers. " f"Received: {value}"
            ) from exc
...
def cutout_video(
    ...
    cutout_center_width = tf.random.uniform(shape=[], minval=0, maxval=video_width, dtype=tf.int32)

    cutout_center_depth = tf.random.uniform(shape=[], minval=0, maxval=video_depth, dtype=tf.int32)
    ...
    padding_dims = [[back_pad, forward_pad], [lower_pad, upper_pad], [left_pad, right_pad]]
    mask = tf.pad(tf.zeros(cutout_shape, dtype=video.dtype), padding_dims, constant_values=1)
    mask = tf.expand_dims(mask, -1)
    num_channels = tf.shape(video)[-1]
    mask = tf.tile(mask, [1, 1, 1, num_channels])
    video = tf.where(tf.equal(mask, 0), tf.ones_like(video, dtype=video.dtype) * replace, video)
    return video
...
def sharpness(image: tf.Tensor, factor: float) -> tf.Tensor:
    ...
    if orig_image.shape.rank == 3:
        kernel = (
            tf.constant([[1, 1, 1], [1, 5, 1], [1, 1, 1]], dtype=tf.float32, shape=[3, 3, 1, 1])
            / 13.0
        )
        ...
        kernel = (
            tf.constant([[1, 1, 1], [1, 5, 1], [1, 1, 1]], dtype=tf.float32, shape=[1, 3, 3, 1, 1])
            / 13.0
        )
        strides = [1, 1, 1, 1, 1]
        ...
        degenerates = [
            tf.nn.conv3d(channel, kernel, strides, padding="VALID", dilations=[1, 1, 1, 1, 1])
...
def unwrap(image: tf.Tensor, replace: int) -> tf.Tensor:
    ...
    image = tf.slice(image, [0] * image.shape.rank, tf.concat([image_shape[:-1], [3]], -1))
    return image
...
def _apply_bbox_augmentation_wrapper(
    ...
    if func_changes_bbox:
        augmented_image, bbox = tf.cond(
            should_apply_op, lambda: augmentation_func(image, bbox, *args), lambda: (image, bbox)
        )
    else:
...
def _check_bbox_area(min_y, min_x, max_y, max_x, delta=0.05):
    ...
    min_x, max_x = tf.cond(
        tf.equal(width, 0.0), lambda: _adjust_bbox_boundaries(min_x, max_x), lambda: (min_x, max_x)
    )
    return min_y, min_x, max_y, max_x
...
def level_to_arg(cutout_const: float, translate_const: float):
    ...
    args = {
        ...
        "Grayscale": no_arg,
        # pylint:disable=g-long-lambda
        "Gaussian_Noise": lambda level: _gaussian_noise_level_to_arg(level, translate_const),
        # pylint:disable=g-long-lambda
        "TranslateX_BBox": lambda level: _translate_level_to_arg(level, translate_const),
        "TranslateY_BBox": lambda level: _translate_level_to_arg(level, translate_const),
        # pylint:enable=g-long-lambda
    ...
    def _make_tf_policies(self):
        ...
        for policy in self.policies:
            ...
            for policy_info in policy:
                ...
                assert_ranges.append(tf.Assert(tf.less_equal(level, int(_MAX_LEVEL)), [level]))
            ...
            def make_final_policy(tf_policy_):
                ...
                def final_policy(image_, bboxes_):
                    for func, prob, args in tf_policy_:
                        image_, bboxes_ = _apply_func_with_prob(func, image_, bboxes_, args, prob)
...
class RandAugment(ImageAugment):
    ...
    def __init__(
        ...
        if exclude_ops:
            self.available_ops = [op for op in self.available_ops if op not in exclude_ops]
            ...
            for _ in range(self._trials):
                if not is_trial_successfull:
                    erase_area = tf.random.uniform(
                        shape=[], minval=area * self._min_area, maxval=area * self._max_area
                    )
                    aspect_ratio = tf.math.exp(
                        tf.random.uniform(
                            shape=[], minval=self._min_log_aspect, maxval=self._max_log_aspect
                        )
                    )
