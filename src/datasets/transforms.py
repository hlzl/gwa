# Forked from https://github.com/google-research/big_vision/blob/main/big_vision/pp/ops_image.py
#
...
"""Image-centric preprocessing transformations.

All preprocessing transformations should return a data processing functor. A data
is represented as a dictionary of (TF) tensors. The functors output a modified dictionary.
"""
...
def get_value_range(dtype=tf.float32):
    """Scale to range [-1, 1]."""
...
def get_resize(size, method="bilinear", antialias=False):
    ...
    def _resize(image):
        ...
        # In particular it was not equivariant with rotation and lead to the network to learn
        # a shortcut in self-supervised rotation task, if rotation was applied after resize.
        dtype = image.dtype
...
def get_resize_long(longer_size, method="area", antialias=True):
    ...
    def _resize_long(image):  # pylint: disable=missing-docstring
        h, w = tf.shape(image)[0], tf.shape(image)[1]
        factor = tf.cast(longer_size, tf.float32) / tf.cast(tf.maximum(h, w), tf.float32)
        return _resize_factor(image, factor, method=method, antialias=antialias)

    return _resize_long


def get_inception_crop(size=None, area_min=5, area_max=100, method="bilinear", antialias=False):
    ...
    def _inception_crop(image):  # pylint: disable=missing-docstring
        ...
        if size:
            crop = get_resize(size, method, antialias)(crop)
        return crop
...
def get_decode_jpeg_and_inception_crop(
    ...
    def _inception_crop(image_data):  # pylint: disable=missing-docstring
        ...
        if size:
            image = get_resize(size, method, antialias)(image)
